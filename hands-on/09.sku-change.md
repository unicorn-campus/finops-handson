...
6. CPU·디스크·네트워크 사용량 차트에서 `95th`를 선택하고 순간 피크는 `Max`로 확인함.  
7. `Available Memory` 차트에서는 `5th`를 선택하고 순간 최저 가용량은 `Min`으로 확인함.  
8. Logical Disk Performance 표의 `P95 IOPs READ`, `P95 IOPs WRITE`, `P95 IOPs TOTAL` 확인함.
...
| 화면 지표 | 지속적인 고사용량 판단 | 순간 최악값 |  
|---|---|---|  
| CPU Utilization | `95th` | `Max` |  
| Memory Utilization 또는 Used Memory | `95th` | `Max` |  
| Available Memory | `5th` | `Min` |  
| Disk IOPS·Network | `95th` | `Max` |  

### 5. Log Analytics에서 P95 검증

1. Azure Portal 상단 검색에서 **Log Analytics 작업 영역** 검색함.  
2. `law-vm-b2b32-p95` 선택함.  
3. 작업 영역 왼쪽 메뉴에서 **로그** 선택함.  
4. `Log Analytics 시작` 안내창이 표시되면 닫음.  
5. 쿼리 편집기 우측 상단의 `단순 모드`를 선택하고 `KQL 모드`로 변경함.  
6. 아래 쿼리 중 하나를 편집기에 붙여넣고 **실행** 또는 `Shift+Enter` 선택함.

![Log Analytics 작업 영역의 로그 KQL 편집기](images/monitor-p95-16-log-analytics-menu.png)
