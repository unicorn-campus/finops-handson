...
| 스토리지 계정 이름 | `stsmart{본인ID}` |  
| 지역 | `(Asia Pacific) Korea Central` |  
| 기본 서비스 | `Azure Blob Storage 또는 Azure Data Lake Storage` |  
| 성능 | `표준` |  
| 중복도 | `ZRS` |

※ Smart Tier는 영역 중복(ZRS·GZRS·RA-GZRS) 계정만 지원함. LRS·GRS는 지원 대상이 아니며,  
설정 후 LRS·GRS로 중복도 전환도 불가함.
...
### 12.3 ZRS 추가 단가와 자동 계층화 절감의 트레이드오프

Smart Tier는 영역 중복(ZRS 계열)이 필수이므로, LRS 대비 저장 단가 상승분을 자동 계층화 절감이  
상쇄하는지 확인 필요함. Korea Central·General Block Blob v2 종량제 소매가 기준(Retail Prices API 조회):

| 중복도 | Hot 저장 단가 (GB·월) | LRS 대비 |  
|---|---|---|  
| LRS | $0.020 | 기준 |  
| ZRS | $0.025 | +25% |  
| GRS | $0.040 | +100% |

- 원래 LRS로 충분한 워크로드면 Smart Tier 도입 시 저장 단가가 25% 상승함  
- 재해 복구 목적으로 GRS를 쓰던 계정이면 ZRS 전환으로 오히려 저렴해짐  
- Hot ZRS($0.025) → Cool ZRS($0.0138) 자동 이동 시 약 45% 절감 →  
  데이터의 상당 부분이 비활성이면 LRS 대비 +25% 단가를 상쇄하고도 남음  
- 대부분 Hot에 머무는 활성 데이터면 Smart Tier + ZRS가 단순 LRS Hot보다 비쌀 수 있음
...
[retail-prices]: https://learn.microsoft.com/rest/api/cost-management/retail-prices/azure-retail-prices
