...
- [Azure Cost analysis 범용 비용 분석 가이드](#azure-cost-analysis-범용-비용-분석-가이드)
  - [0. 개요와 사용법](#0-개요와-사용법)
    - [0.1 목적](#01-목적)
    - [0.2 대상·전제](#02-대상전제)
    - [0.3 분석 철학: "그룹화 = 지도, 필터 = 확대경"](#03-분석-철학-그룹화--지도-필터--확대경)
    - [0.4 분석 목적 3분류](#04-분석-목적-3분류)
  - [1. 사전 준비: 컨트롤 이해](#1-사전-준비-컨트롤-이해)
    - [1.1 분석기간 결정 규칙 (변수 `{분석기간}`)](#11-분석기간-결정-규칙-변수-분석기간)
    - [1.2 표준 태그키 결정 규칙 (변수 `{표준태그키}`)](#12-표준-태그키-결정-규칙-변수-표준태그키)
    - [1.3 분석 프레임워크: 9축 관계와 그룹화 → 필터 연결](#13-분석-프레임워크-9축-관계와-그룹화--필터-연결)
  - [2. 그룹화(Group by) 9축](#2-그룹화group-by-9축)
    - [2.1 None + 월별 — 총액·추이 \& 예산 대비](#21-none--월별--총액추이--예산-대비)
    - [2.2 Service name — 집중 비용원](#22-service-name--집중-비용원)
    - [2.3 Resource group name — 비용 귀속 분포](#23-resource-group-name--비용-귀속-분포)
    - [2.4 Resource type — 원가동인 재확인 (같은 비용, 두 렌즈)](#24-resource-type--원가동인-재확인-같은-비용-두-렌즈)
    - [2.5 Location — 리전 분포](#25-location--리전-분포)
    - [2.6 Tag — 태그 기반 배분·거버넌스](#26-tag--태그-기반-배분거버넌스)
    - [2.7 Pricing Model — 커버리지(온디맨드 vs 약정)](#27-pricing-model--커버리지온디맨드-vs-약정)
    - [2.8 Service name + 월별·일별 — 이상 신호(급증 원인 attribution)](#28-service-name--월별일별--이상-신호급증-원인-attribution)
    - [2.9 최적화 블레이드 — Advisor 권장](#29-최적화-블레이드--advisor-권장)
  - [3. 필터(Filter) 드릴다운 패턴 (비용 절감 3그룹)](#3-필터filter-드릴다운-패턴-비용-절감-3그룹)
    - [비용 절감 관점 분석](#비용-절감-관점-분석)
      - [3.1 약정 커버리지·여지 격리 (S1)](#31-약정-커버리지여지-격리-s1)
      - [3.2 dev/test 스케줄링 여지 (S2)](#32-devtest-스케줄링-여지-s2)
      - [3.3 프로젝트 타임라인 — 종료 잔존 회수 (S3)](#33-프로젝트-타임라인--종료-잔존-회수-s3)
      - [3.4 스토리지 계층 규모 (S4)](#34-스토리지-계층-규모-s4)
      - [3.5 스냅샷·백업 보존 과다 (S5)](#35-스냅샷백업-보존-과다-s5)
      - [3.6 리전 스프롤 (S6)](#36-리전-스프롤-s6)
      - [3.7 데이터 전송(egress) 유발원 (S7)](#37-데이터-전송egress-유발원-s7)
      - [3.8 고아 리소스 검증 (S8)](#38-고아-리소스-검증-s8)
    - [리소스 배분 분석](#리소스-배분-분석)
      - [3.9 미태깅 범인 식별 (A1)](#39-미태깅-범인-식별-a1)
      - [3.10 공유 리소스 구성 (A2)](#310-공유-리소스-구성-a2)
      - [3.11 원가동인 귀속 — 프로젝트·서비스 2축 (A3)](#311-원가동인-귀속--프로젝트서비스-2축-a3)
    - [추세 분석](#추세-분석)
      - [3.12 예산 소진율·번레이트 (T1)](#312-예산-소진율번레이트-t1)
      - [3.13 MoM 증가율 추적 (T2)](#313-mom-증가율-추적-t2)
      - [3.14 급증 격리 (T3)](#314-급증-격리-t3)
      - [3.15 축 교차 원칙](#315-축-교차-원칙)
  - [4. 종합](#4-종합)
    - [4.1 표준 시퀀스: 규모 → 집중 비용원 → 귀속처 파악 → 원인 규명 → 조치](#41-표준-시퀀스-규모--집중-비용원--귀속처-파악--원인-규명--조치)
    - [4.2 액션 시퀀스 표 (재현용)](#42-액션-시퀀스-표-재현용)
    - [4.3 체크리스트 (릴리스 전 필수 점검)](#43-체크리스트-릴리스-전-필수-점검)
    - [4.4 함정 (Pitfalls)](#44-함정-pitfalls)
    - [4.5 Inform → Optimize 브릿지](#45-inform--optimize-브릿지)
...
![S1 그룹화 축·가설 및 분석대상·분석방법](images/cost-analysis-s1-detail.png)
...
| 필터 | 그룹화 | 세분성 | 차트 | 분석 방법 |
|---|---|---|---|---|
| Pricing Model = OnDemand | Service name | 월별 | 테이블 | 온디맨드 상위 서비스 중 월별 소비가 **꾸준한(steady-state)** 것이 약정 1순위.<br>2.1 소비 패턴·2.9 Advisor 예약 추천과 교차. |

**주의:** 간헐적(bursty) 소비는 약정 시 유휴월 낭비 → **무조건 약정은 오답**. 정확 할인율·손익분기는 Advisor·약정 계산기로 확정함(Cost Analysis는 여지 발굴까지).

#### 3.2 dev/test 스케줄링 여지 (S2)

![S2 그룹화 축·가설 및 분석대상·분석방법](images/cost-analysis-s2-detail.png)
...
| 필터 | 그룹화 | 세분성 | 차트 | 분석 방법 |
|---|---|---|---|---|
| Tag(`env` 키) = dev·staging·test | Resource type | 일별(피크월 범위) | 세로 막대형(누적) 또는 테이블 | 일별 막대가 **주말·야간에도 평일과 동일**하면 상시 가동(스케줄링 미적용).<br>평일 주간만 과금되면 이미 스케줄링됨. |

**주의:** `env` 미태깅이 크면 이 분석 자체가 불가 → **배분 전제(3.9/A1) 선행** 필요. 일별은 장기 범위에서 막히므로 **피크월로 좁힌 뒤** 사용함.

#### 3.3 프로젝트 타임라인 — 종료 잔존 회수 (S3)

![S3 그룹화 축·가설 및 분석대상·분석방법](images/cost-analysis-s3-detail.png)
...
| 필터 | 그룹화 | 세분성 | 차트 | 분석 방법 |
|---|---|---|---|---|
| Tag(`project` 키) = (특정 프로젝트) | None | 월별 | 세로 막대형(누적) 또는 테이블 | **최근월에도 과금이 지속**되는데 프로젝트가 명목상 종료·비활성이면 → **잔존 리소스 회수 대상**(고아 리소스 3.8/S8과 교차 확인).<br>최근월이 0에 가까우면 **이미 자연 종료** → 회수 여지 없음(지금 최적화해도 절감 0). 소비 집중월을 실제 데이터로 확인. |
...
![S4 그룹화 축·가설 및 분석대상·분석방법](images/cost-analysis-s4-detail.png)
...
| 필터 | 그룹화 | 세분성 | 차트 | 분석 방법 |
|---|---|---|---|---|
| Resource type = microsoft.storage/*, microsoft.compute/disks | **Meter**(Resource type 아님) | 없음(누적) | 테이블 | **Resource type 그룹화는 티어를 드러내지 않음**(ARM 리소스 타입에는 Hot/Cool/Premium 구분이 없음) → 반드시 **Meter로 그룹화**해야 티어가 미터명에 노출됨.<br>미터명 판독: Managed Disk는 `P4/P30 LRS Disk`(Premium SSD)·`E10 LRS Disk`(Standard SSD)·`S4 LRS Disk`(Standard HDD), Blob은 `Hot/Cool/Archive LRS Data Stored`로 표기됨.<br>Hot/Premium 미터에 큰 금액이 몰리면 계층화 후보. 대용량 상위 계정·디스크를 식별함. |
...
![S5 그룹화 축·가설 및 분석대상·분석방법](images/cost-analysis-s5-detail.png)
...
| 필터 | 그룹화 | 세분성 | 차트 | 분석 방법 |
|---|---|---|---|---|
| Resource type = `microsoft.compute/snapshots`(디스크 스냅샷) 또는 `microsoft.recoveryservices/vaults`(백업 볼트) | None | 월별 | 테이블 | 월별로 계속 증가하면 보존정책 없이 누적 중.<br>감소 없이 우상향이면 정리 대상. |
...
![S6 그룹화 축·가설 및 분석대상·분석방법](images/cost-analysis-s6-detail.png)
...
| 필터 | 그룹화 | 세분성 | 차트 | 분석 방법 |
|---|---|---|---|---|
| — | Location | 없음(누적) | 테이블 | 주력 리전 외 소액 리전이 다수면 **테스트 잔재·미승인 배포** 가능. |
| Location = (주력 외 소액 리전) | Resource type | 없음(누적) | 테이블 | 각 리전의 Resource type으로 정리 대상 확인. |

**주의:** 데이터 레지던시·DR 요건상 **의도적 분산**일 수 있음 → 정리 전 목적 확인. 리전 이전은 전송비·다운타임을 동반함.

#### 3.7 데이터 전송(egress) 유발원 (S7)

![S7 그룹화 축·가설 및 분석대상·분석방법](images/cost-analysis-s7-detail.png)
...
| 필터 | 그룹화 | 세분성 | 차트 | 분석 방법 |
|---|---|---|---|---|
| Meter category = `Bandwidth` | **Resource type**(Service name 아님) | 월별 | 테이블 | egress 상위 리소스 유형(예: VM·Load Balancer·VPN Gateway·CDN)이 **크로스리전·CDN 미적용·중복 복제**인지 확인.<br>월별 추세로 급증 여부 파악. |
...
![S8 그룹화 축·가설 및 분석대상·분석방법](images/cost-analysis-s8-detail.png)

**규명 대상 · FinOps: Workload Optimization**  
고아 의심 리소스(미연결 디스크·미할당 공인 IP·유휴 NIC/LB/NAT Gateway 등)를 **효율적으로 발굴**하고, 그 리소스가 **지속 과금인지·과거 일회성인지** 확인해 회수 대상을 판정함.

**수행 (발굴 → 비용 검증 2단계):**  
탐색 리스트·KQL 쿼리·Copilot 프롬프트는 [resource-graph-explore.md](resource-graph-explore.md) 참조.

| 필터 | 그룹화 | 세분성 | 차트 | 분석 방법 |
|---|---|---|---|---|
| — (Azure Resource Graph Explorer로 비연결 상태 질의) | — | — | — | **발굴(상태 기반, 효율적)**: 디스크 `managedBy`가 빈 값·공인 IP 미할당 등을 질의하거나, **Orphaned Resources 워크북**·**Advisor 비용 권장**으로 고아 후보를 일괄 추출. |
| Resource type = (해당 유형) | None | 월별 | 테이블 | **비용 검증**: 최근 월에도 계속 과금되면 **회수 대상 고아 리소스**.<br>과거 단일월만 발생하고 이후 0이면 **이미 사라진 일회성**(회수 대상 아님). |
...
![A1 그룹화 축·가설 및 분석대상·분석방법](images/cost-analysis-a1-detail.png)
...
| 필터 | 그룹화 | 세분성 | 차트 | 분석 방법 |
|---|---|---|---|---|
| Tag(배분 키) = "태그 없음" | Resource group name | 없음(누적) | 테이블 | 필터 후 상위 RG가 미태깅의 몇 %인지 계산.<br>단일 대형 RG(공유 인프라)가 주범인지, 여러 RG에 얇게 분산된 롱테일인지 구분. |
...
![A2 그룹화 축·가설 및 분석대상·분석방법](images/cost-analysis-a2-detail.png)
...
| 필터 | 그룹화 | 세분성 | 차트 | 분석 방법 |
|---|---|---|---|---|
| Resource group name = (위에서 식별한 공유 RG) | Service name | 없음(누적) | 테이블 | 상위 서비스들이 **모든 팀이 공통 호출하는 플랫폼**(예: 추론·API 게이트웨이·검색)인지 확인 → 사용량 기반 chargeback 대상. |

**주의:** 공유비용은 균등 분할 또는 사용량(호출 수·토큰) 기반 배분이 필요함 → 단일 태그로 귀속 불가.

#### 3.11 원가동인 귀속 — 프로젝트·서비스 2축 (A3)

![A3 그룹화 축·가설 및 분석대상·분석방법](images/cost-analysis-a3-detail.png)
...
| 필터 | 그룹화 | 세분성 | 차트 | 분석 방법 |
|---|---|---|---|---|
| — (없음) | Tag(`project` 키), Tag(`workload` 키) 각각 | 없음(누적) | 테이블 | **0. 선행 조회(전체 순위부터 확인)**: 리소스 유형으로 좁히기 전에 **전체 범위에서 어느 프로젝트·어느 워크로드가 비용을 가장 많이 쓰는지** 순위를 먼저 파악함. |
| Resource type = (최대 유형) | Tag(`project` 키) | — | — | **1. 프로젝트 축(원가동인 교차)**: 0번의 상위 프로젝트가 이 원가동인에서도 상위인지 대조해 **기간 한정 과제별 귀속**을 확정. |
| Resource type = (최대 유형) | Tag(`workload` 키) | — | — | **2. 워크로드 축(원가동인 교차)**: 0번의 상위 워크로드가 이 원가동인에서도 상위인지 대조해 **상시 서비스별 귀속**을 확정. |
| — | — | — | — | 두 축을 교차해야 원가동인 주체를 놓치지 않음 — `project` 축에서 **"태그 없음" 비중이 크면** 원가동인이 기간 한정 과제가 아니라 **상시 서비스**일 가능성이 높으므로, `workload` 축으로 관점을 바꿔 다시 확인함.<br>0번 선행 조회의 순위와 1·2번 교차 결과가 서로 맞는지도 함께 봄. |
...
![T1 그룹화 축·가설 및 분석대상·분석방법](images/cost-analysis-t1-detail.png)
...
| 필터 | 그룹화 | 세분성 | 차트 | 분석 방법 |
|---|---|---|---|---|
| — | None | 월별 | 세로 막대형(누적) | 비용 관리 > **예산(Budgets)**·Forecast와 대조.<br>최근 월 소비가 월 예산의 몇 %인지, 잔여 예산으로 몇 개월 버티는지(번레이트) 계산.<br>Forecast 초과선 교차월 확인. |

**주의:** 당월은 미확정(변동) → 소진율은 **경향으로만** 해석. 실제/예측 예산 알림 룰 설정 여부 점검. 간헐적 소비는 Forecast 산출 불가할 수 있음(2.1).

#### 3.13 MoM 증가율 추적 (T2)

![T2 그룹화 축·가설 및 분석대상·분석방법](images/cost-analysis-t2-detail.png)
...
| 필터 | 그룹화 | 세분성 | 차트 | 분석 방법 |
|---|---|---|---|---|
| — | Service name | 월별 | 세로 막대형(그룹화) 또는 테이블 | 서비스별 월간 증감 계산.<br>**3개월 이상 연속 상승**하는 서비스는 구조적 증가(급증과 다름).<br>신규 등장·꾸준히 우상향 서비스를 표시. |

**주의:** MoM은 **긴급도 신호이지 절감액이 아님** → 상승 원인은 급증 격리(3.14/T3)로 규명. 계절성(연말·배포주기)과 구조적 증가를 혼동 금지.

#### 3.14 급증 격리 (T3)

![T3 그룹화 축·가설 및 분석대상·분석방법](images/cost-analysis-t3-detail.png)
...
| 필터 | 그룹화 | 세분성 | 차트 | 분석 방법 |
|---|---|---|---|---|
| Service name = (급증 서비스) | Resource group name | 없음(누적) | 테이블 | **1. RG 격리(지점)**: 어느 RG가 급증을 차지하는지 격리. |
| Service name = (급증 서비스), 기간=피크월(단월) | — | 일별 | 테이블(또는 세로 막대형) | **2. 발생일 정밀(시점)**: 급증이 특정 일(1회성)인지 월 전반(지속)인지, 정확한 발생일 확정. |
| — | — | — | — | 필터 후 단일 RG가 전액을 차지하면 **일회성 배포·게이트웨이 가동** 등으로 해석.<br>2.8의 월별→일별 attribution 결과와 정합 확인. |
