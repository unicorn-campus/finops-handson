...
| Archive | 장기 보관하며 즉시 접근이 필요 없는 데이터 | 180일 | 불가능 | 읽기 전 재수화 필요, 시간과 비용 발생 |  
...
![스토리지 계정 검토](images/blob-tiering-03-storage-review.png)

![스토리지 계정 배포 완료](images/blob-tiering-04-storage-deployed.png)
...
Archive는 온라인에서 즉시 읽을 수 없고 재수화가 필요하므로 이 단계에서는 설정하지 않음.
...
7. Archive 조건의 `마지막에 재수화된 Blob 건너뛰기`를 선택 상태로 유지.  
8. 재수화 후 유예 기간을 기본값 `7일`로 유지.  
9. `다음` 선택.  

재수화된 Blob 건너뛰기 조건은 최근 복원한 Blob이 즉시 다시 Archive로 이동하는 상황을 줄이는 안전장치임.
...
포털에서 저장한 실제 정책에는 최근 재수화 후 7일 동안 Archive 이동을 건너뛰는 조건도 포함됨.
...
새 규칙 또는 변경된 규칙이 적용되는 데 최대 24시간이 걸릴 수 있으며,  
...
```text
총비용 = 저장 비용 + 작업 비용 + 데이터 조회 비용 + 조기 삭제 비용 + 재수화 비용
```
...
- 월별 비용과 재수화 건수를 검토하여 규칙 반복 개선  

### 11.3 Archive 사용 판단

Archive Blob은 오프라인 상태이므로 읽기 전에 Hot 또는 Cool 계층으로 재수화해야 함.  
재수화는 우선순위와 데이터 크기에 따라 시간이 걸리며 관련 작업·조회 비용이 발생함.  
...
- Hot 또는 Cool 계층으로 재수화 요청  
- 재수화 완료 전 읽기 불가  
- 재수화 시간과 비용을 운영 계획에 반영  
...
- [Archive Blob 재수화 개요][archive-rehydrate]  
