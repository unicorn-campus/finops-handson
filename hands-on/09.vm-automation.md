...
- `Standard_D2_v6` 크기의 Linux VM 생성
...
> [!NOTE]
> 실습 목표 SKU는 `Standard_D2_v6`임.  
> 실제 검증 화면에서는 사용자가 선택한 `Standard_D2s_v6`로 VM이 생성됨.  
> 두 SKU는 서로 다른 크기이므로 수강 환경에서 `D2_v6`가 제공되면 `Standard_D2_v6`를 선택함.
...
2. 검색 상자에 `D2 v6`를 입력함.

3. `Standard_D2_v6`를 선택함.
