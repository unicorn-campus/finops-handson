...
관찰 결과 가정: 현재 Standard_D4s_v5(4 vCPU / 16GB), CPU P95 약 20%(여유 80%), 메모리 사용 P95 약 20%(약 3.2GB, 여유 80%).

> SKU 스펙은 Microsoft Learn VM 사이즈 문서 기준으로 대조 검증함(2026-07-22 확인).  
> [Dsv5-series](https://learn.microsoft.com/en-us/azure/virtual-machines/sizes/general-purpose/dsv5-series) ·
> [Bv1-series(B2s/B2ms 기준 CPU 성능)](https://learn.microsoft.com/en-us/azure/virtual-machines/sizes/general-purpose/bv1-series)

> 후보 축소 시 사용률(%)은 **절대 사용량 고정, 분모(vCPU·메모리)만 축소**되는 것으로 환산함.  
> 예: CPU 20%(4vCPU 기준) → 절대 사용량 0.8 vCPU-eq → 2vCPU 대비 40%.  
> 메모리 20%(16GB 기준) → 절대 사용량 약 3.2GB → 8GB 대비 40%, 4GB 대비 80%.

| 구분 | SKU | vCPU / 메모리 | 월 예상 비용(예시) | 헤드룸(CPU / 메모리) | 리스크 · 다운타임 |
|------|-----|---------------|--------------------|----------------------|-------------------|
| 현재 | Standard_D4s_v5 | 4 / 16GB | 기준(100%) | CPU 80% / 메모리 80% 유휴(과잉) | 없음(현행 유지) |
| 후보 A(안전) | Standard_D2s_v5 | 2 / 8GB | 약 50% 수준 | CPU 20%→40%(여유 60%) / 메모리 20%→40%(여유 60%) | 크기 변경 재부팅 1회 |
| 후보 B(권장) | Standard_B2ms | 2 / 8GB(버스트, 기준 CPU 30%) | 약 35% 수준 | CPU 여유 60%(단, 버스트 기준 30% 초과 사용 · 지속 고부하 부적합) / 메모리 여유 60% | 재부팅 + 버스트 크레딧 소진 리스크 |
| 후보 C(최소 · 경계) | Standard_B2s | 2 / 4GB(버스트, 기준 CPU 20%) | 약 20% 수준 | CPU 여유 60% / **메모리 여유 20%(하한 경계, 여유 폭 매우 좁음)** | 재부팅 + 메모리 여유 부족 위험 + 크레딧 소진 |

- 후보 A: CPU·메모리 모두 헤드룸 60% 확보(기준 20 ~ 30% 이상 대비 여유 충분) → 범용(D-계열) 유지로 성능 예측 용이한 안정적 축소안.
- 후보 B: 버스트(B-계열)로 비용 최저 수준이나, 실사용(40%)이 기준 CPU 성능(30%)을 지속 초과 → 크레딧 소진 시 성능 저하 리스크.  
  간헐적 저부하 워크로드에만 적합.
- 후보 C: **CPU 헤드룸(60%)만 보면 여유로워 보이나, 메모리 헤드룸은 20%로 STEP 3 선정 로직의 최소 기준(20 ~ 30%) 하한에 걸쳐 있음.**  
  탈락은 아니지만 향후 부하 증가·관찰 오차를 흡수할 여유가 거의 없는 **경계선 후보** — CPU만 보고 축소하면 이런 여유 부족을 놓치기 쉬움.  
  (참고: 메모리 P95가 20%보다 조금만 높았어도(예: 25%) 4GB SKU는 헤드룸 기준 미달로 자동 탈락했을 사례임)
- **주의(함정 "CPU만 보기")**: 후보 C처럼 CPU만 보고 축소하면 메모리 부족을 놓칠 수 있음.  
  메모리 P95가 더 높았다면(예: 12GB 사용) 8GB SKU(후보 A · B)도 동일한 이유로 탈락했을 것임 — 두 지표를 함께 봐야 하는 이유.
