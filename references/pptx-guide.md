...
**규칙**: `fontSize` 값을 직접 숫자로 쓰지 말고 반드시 `fs14()` 함수를 경유할 것.
...
function headerBar(slide, { x, y, w, text, accent = false }) {
  ...
  slide.addText(text, { x, y, w, h: 0.5, align: "center", color: "FFFFFF",
    bold: true, fontSize: fs14(20), fontFace: FONT });
}
```
...
function numBadge(slide, { x, y, n, color = C.blue, size = 0.4 }) {
  ...
  slide.addText(String(n), { x, y, w: size, h: size, align: "center", valign: "middle",
    color: "FFFFFF", bold: true, fontSize: fs14(16), fontFace: FONT });
}
```
...
slide.addTable(
  ...
  { x: 0.55, y: 1.9, w: 9, colW: [2, 7], fontSize: fs14(14), fontFace: FONT,
...
{ text: "안녕하세요", options: { fontFace: FONT, fontSize: fs14(16) } }
```
...
#### 6-14. 생성 후 자가 검증 체크리스트

| # | 검증 항목 | 방법 | 합격 기준 |
|---|----------|------|----------|
| 1 | 최소 폰트 크기 | 스크립트 내 모든 fontSize 값 확인 | 14pt 이상 (fs14 경유) |
